...
def main():
    ...
    style.map(
        "Custom.Vertical.TScrollbar",
        background=[
            ("active", "#252525"),  # при наведении мыши
            ("pressed", "#1a1a1a"),
        ],  # при зажатии
    ...
    def on_profile_click(event):
        ...
        if content:
            ...
            text_sources.insert(
                tk.END,
                f"WARNING: source.txt file not found in the {profile_name} profile, make sure it exists",
            )
    ...
    def check_download_status():
        ...
        global is_downloading, spinner_idx
        ...
        try:
            ...
            if total > 0:
                ...
                progress_label.config(
                    text=f"[{spinner_char}] {downloaded} / {total} ({percent}%)"
                )
    ...
    def start_download():
        ...
        DOWNLOAD_ALL_btn.config(
            state=tk.DISABLED, text="DOWNLOADING...", cursor="arrow"
        )
